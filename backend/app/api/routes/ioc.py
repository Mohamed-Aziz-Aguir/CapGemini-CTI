...
@router.post("/analyze")
async def analyze_ioc(data: IOCRequest):
    ...
    if results["otx"] is None:
        results["otx"] = await get_info_from_otx(ioc_value)

    if results["virustotal"] is None:
        results["virustotal"] = await get_info_from_virustotal(ioc_value)
